...
export default function ColumnEditDelete({ column, onUpdated }) {
  ...
  const handleUpdate = async e => {
    e.preventDefault();
    setLoading(true);
    await axios.put(`https://ai-taskmate-k7qw.onrender.com/api/columns/${column._id}`, { name, order: column.order });
    setEditing(false);
    setLoading(false);
    onUpdated && onUpdated();
  };
  ...
  const handleDelete = async () => {
    setLoading(true);
    await axios.delete(`https://ai-taskmate-k7qw.onrender.com/api/columns/${column._id}`);
    setLoading(false);
    onUpdated && onUpdated();
  };
